...
export class RoundService {
  ...
  async setRemainingTimeTeam1(id: string, seconds: number): Promise<boolean> {
    const { error } = await this.supabaseService.client
      .from('rounds')
      .update({ remaining_time_team1: seconds })
      .eq('id', id);
    return !error;
  }

  async setRemainingTimeTeam2(id: string, seconds: number): Promise<boolean> {
    const { error } = await this.supabaseService.client
      .from('rounds')
      .update({ remaining_time_team2: seconds })
      .eq('id', id);
    return !error;
  }
